...
/// to make sure timestamp will get updated independently of other users and transactions
...
contract Phase1 is Script {
    ...
    function run() external {
        ...
        auctioner.create("Asset", "AST", "https:", 0.01 ether, 100, 10, block.timestamp + 3 days, block.timestamp + 10 days, BROKER, 500, 50);
        vm.stopBroadcast();

        /// @dev OPENED AUCTION (id: 1)
        vm.startBroadcast(adminKey);
        auctioner.create("Asset", "AST", "https:", 0.01 ether, 100, 10, block.timestamp + 1 days, block.timestamp + 7 days, BROKER, 500, 50);
        auctioner.stateHack(1, 2);
        vm.stopBroadcast();

        /// @dev OPENED AUCTION WITH BUYERS (id: 2)
        vm.startBroadcast(adminKey);
        auctioner.create("Asset", "AST", "https:", 0.01 ether, 100, 10, block.timestamp + 1 days, block.timestamp + 70 days, BROKER, 500, 50);
...
contract Phase2 is Script {
    ...
    function run() external {
        ...
        vm.startBroadcast(adminKey);
        auctioner.create("Asset", "AST", "https:", 0.01 ether, 30, 10, block.timestamp + 1 days, block.timestamp + 7 days, BROKER, 500, 50);
